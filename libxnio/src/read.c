...
JNIEXPORT jint JNICALL xnio_native(readDirect)(JNIEnv *env, jclass clazz, jint fd, jobject bufferObj, jintArray posAndLimit) {
    jint position, limit;
    ...
    ssize_t res;
    if ((res = read(fd, buffer, limit - position)) == -1) {
        return -errno;
    }
    if (res > 0) {
        position += res;
...
JNIEXPORT jlong JNICALL xnio_native(readDirectScatter)(JNIEnv *env, jclass clazz, jint fd, jobjectArray bufferObjs, jint offs, jint len, jintArray posArray, jintArray limitArray) {
...
JNIEXPORT jint JNICALL xnio_native(readHeap)(JNIEnv *env, jclass clazz, jint fd, jbyteArray bufferObj, jintArray posAndLimit) {
    jint position, limit;
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, posAndLimit, pal, JNI_ABORT);
    ssize_t res;
    if ((res = read(fd, buffer, limit - position)) < 0) {
        (*env)->ReleaseByteArrayElements(env, bufferObj, buffer, JNI_ABORT);
        return -errno;
    }
...
JNIEXPORT jlong JNICALL xnio_native(readHeapScatter)(JNIEnv *env, jclass clazz, jint fd, jobjectArray bufferObjs, jint offs, jint len, jintArray posArray, jintArray limitArray) {
    struct iovec iov[len];
    ssize_t res;
